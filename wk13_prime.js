...
const isPrimeOrNot = (num) => {
	if (num > 1) {
		...
		const nums2inNumber = [...Array(num).keys()];
		nums2inNumber.splice(0, 2);
		// console.log(`[info] Now inspecting the following : ${nums2inNumber}`);
		for (let i = 0; i < nums2inNumber.length; i++) {
			const result = num / nums2inNumber[i];
			// console.log(`[info] Results ${result} and is whole number : ${Number.isInteger(result)}`);
...
console.log(isPrimeOrNot((inNum = inputNumber)));
